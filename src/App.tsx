          ...
          <div className="lg:sticky lg:top-8 h-fit">
            <Card className="bg-[#111111] border-[#222222]">
              <CardHeader className="space-y-1 sm:space-y-2">
                <CardTitle className="text-xl sm:text-2xl text-[#EDEDED]">Cold Call Coach</CardTitle>
                <CardDescription className="text-sm sm:text-base text-[#AAAAAA]">
                ...
                <div className="flex flex-col items-center gap-3 sm:gap-4">
                  <div className="w-full">
                    <label
                      ...
                      className="flex flex-col items-center justify-center w-full h-24 sm:h-32 border-2 border-dashed rounded-lg cursor-pointer bg-[#111111] hover:bg-[#222222] border-[#333333]"
                    >
                      <div className="flex flex-col items-center justify-center px-4 py-3 sm:py-6 text-center">
                        {isAnalyzing ? (
                          <div className="text-center">
                            <div className="animate-spin rounded-full h-6 w-6 sm:h-8 sm:w-8 border-b-2 border-blue-500 mb-2"></div>
                            <p className="text-xs sm:text-sm text-[#AAAAAA]">Analyzing your call...</p>
                          </div>
                        ) : (
                          <>
                            <p className="mb-1 sm:mb-2 text-xs sm:text-sm text-[#AAAAAA]">
              ...
              <>
                  ...
                  <div className="relative">
                    ...
                    <CardContent className="pt-6">
                      <div className="flex items-baseline gap-2 mb-3 sm:mb-4">
                        <h3 className="text-xl sm:text-2xl font-bold text-[#EDEDED]">Overall Score:</h3>
                        <span className={`text-2xl sm:text-3xl font-bold ${getScoreColor(feedback.overallScore.score)}`}>
                          {feedback.overallScore.score}/10
                        </span>
                      </div>
                      <p className="text-sm sm:text-base text-[#EDEDED]">{feedback.overallScore.summary}</p>
                    </CardContent>
                  </div>
                </Card>
                  ...
                  <Card key={index} className="bg-[#111111] border-[#222222]">
                    <CardContent className="pt-6">
                      <div className="flex items-baseline justify-between mb-3 sm:mb-4">
                        <h3 className="text-lg sm:text-xl font-bold text-[#EDEDED]">{section.title}</h3>
                        <span className={`text-base sm:text-lg font-semibold ${getScoreColor(section.data.score)}`}>
                          Score: {section.data.score}/10
                        </span>
                      </div>
                      <ul className="list-disc pl-4 sm:pl-5 space-y-1.5 sm:space-y-2 mb-3 sm:mb-4 text-sm sm:text-base">
                        {section.data.feedback.map((point, i) => (
                          <li key={i} className="text-[#EDEDED]">{point}</li>
                        ))}
                      </ul>
                      {section.suggestion && (
                        <div className="mt-3 sm:mt-4 p-3 sm:p-4 bg-[#1a1a1a] rounded-lg">
                          <p className="font-semibold text-green-400 mb-2 text-sm sm:text-base">{section.suggestionTitle}:</p>
                          <p className="text-[#EDEDED] italic text-sm sm:text-base">{section.suggestion}</p>
                        </div>
                      )}
                      {section.recommendations && section.recommendations.length > 0 && (
                        <div className="mt-3 sm:mt-4 p-3 sm:p-4 bg-[#1a1a1a] rounded-lg">
                          <p className="font-semibold text-blue-400 mb-2 text-sm sm:text-base">Recommendations:</p>
                          <ul className="list-disc pl-4 sm:pl-5 space-y-1.5 sm:space-y-2">
                            {section.recommendations.map((rec, i) => (
                              <li key={i} className="text-[#EDEDED] italic text-sm sm:text-base">{rec}</li>
                            ))}
                          </ul>
                        </div>
                      )}
                ...
                <Card className="bg-[#111111] border-[#222222]">
                  <div className="relative">
                    ...
                    <CardContent className="pt-6">
                      <h3 className="text-lg sm:text-xl font-bold mb-3 sm:mb-4 text-[#EDEDED]">Actionable Takeaways</h3>
                      <div className="space-y-3 sm:space-y-4">
                        <div>
                          <h4 className="font-semibold text-[#EDEDED] mb-2 text-sm sm:text-base">Top Areas for Improvement:</h4>
                          <ul className="list-disc pl-4 sm:pl-5 space-y-1.5 sm:space-y-2">
                            {feedback.actionableTakeaways.improvements.map((improvement, i) => (
                              <li key={i} className="text-[#EDEDED] text-sm sm:text-base">{improvement}</li>
                            ))}
                          </ul>
                        </div>
                        {feedback.actionableTakeaways.scriptExample && (
                          <div className="p-3 sm:p-4 bg-[#1a1a1a] rounded-lg">
                            <h4 className="font-semibold text-purple-400 mb-2 text-sm sm:text-base">Example Script:</h4>
                            <p className="text-[#EDEDED] italic text-sm sm:text-base">{feedback.actionableTakeaways.scriptExample}</p>
                          </div>
                        )}
                      </div>
                    </CardContent>
